...
void setup()
{
  ...
  beat = new BeatDetect(in.bufferSize(), in.sampleRate()); //song.bufferSize(), song.sampleRate());
  ...
  bl = new BeatListener(beat, in); //song);  
...
void sendOSCToResolume(String target, float value) {
  count++;
  //String target = "/layer"+ layerNr + "/clip" + clipNr + "/connect";
  println(count + " sending '"+value+"' to target " + target);
  OscMessage myMessage = new OscMessage(target);
  myMessage.add(value); //count); //1);
...
void draw()
{
  ...
  /*if (beat.isRange(1, 10, 4)) {
    sendOSCToResolume("/bass_energy", );
  }*/
  ...
  if (kickSize > 30 ) sendOSCToResolume("/kick_energy", kickSize/100.0f);
  if (snareSize > 30 ) sendOSCToResolume("/snare_energy", snareSize/100.0f);
  if (hatSize > 30 ) sendOSCToResolume("/hat_energy", hatSize/100.0f);
