...
public class ImageListScene extends SimpleScene {
	...
	public HashMap<String,Object> collectParameters() {
		...
		params.put(this.getPath()+"/current_image_index", this.getCurrentImageFilename(ImageRepository.IR)); //current_image_index);
		return params;
	}
	...
	private int getIndexForFilename(String payload) {
		return this.filenames.indexOf(payload);
	}
	...
	public boolean setupFilters () {
		...
		this.addFilter(new ImageListDrawer(this).setFilenames(this.getFilenames()).setCurrentIndex(5).setNumBlobs(BLOBCOUNT/*200*/).setFilterName("ImageListDrawer")).setAlias_out("out"); //.nextMode());
	...
	private String getCurrentImageFilename(ImageRepository iR) {
		...
		return filenames.get(current_image_index);
	}
	...
	public PImage getCurrentImage (ImageRepository IR) {
     ...
     current_image_index = 0;
		...
		return IR.getImageForFilename(getCurrentImageFilename(ImageRepository.IR), this.w, this.h);
